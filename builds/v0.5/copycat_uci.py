#!/usr/bin/env python
# copycat_uci.py - UCI launcher for Copycat Chess Engine v0.5 BETA
import sys
import os
import chess
import time
...
class UCIEngine:
    def __init__(self):
        self.name = "Copycat Chess Engine v0.5 BETA"
    ...
    def initialize_engine(self):
        ...
        try:
            ...
            # Try RL models first (v0.5 BETA feature)
            ...
            if os.path.exists(rl_actor_path) and os.path.exists(rl_critic_path):
                ...
                # For now, fall back to evaluation AI since RL integration might need special handling
                
            # Look for traditional models
            model_paths = [
                os.path.join(script_dir, "v7p3r_chess_ai_model.pth"),
                os.path.join(script_dir, "..", "v0.3_copycat_enhanced_ai", "v7p3r_chess_ai_model.pth"),
                os.path.join(script_dir, "..", "v0.2_copycat_eval_ai", "v7p3r_chess_ai_model.pth"),
                os.path.join(script_dir, "..", "v0.1_copycat_ai", "v7p3r_chess_ai_model.pth"),
                os.path.join(script_dir, "..", "..", "rl_actor_model.pth")  # Main directory
            ]
            
            vocab_paths = [
                os.path.join(script_dir, "move_vocab.pkl"),
                os.path.join(script_dir, "..", "v0.3_copycat_enhanced_ai", "move_vocab.pkl"),
                os.path.join(script_dir, "..", "v0.2_copycat_eval_ai", "move_vocab.pkl"),
                os.path.join(script_dir, "..", "v0.1_copycat_ai", "move_vocab.pkl")
            ]
            ...
            if not model_path:
                print("info string Model file not found in any location")
                return False
                
            if not vocab_path:
                print("info string Vocabulary file not found in any location")
                return False
...
def main():
    """Launch the Copycat Chess Engine v0.5 BETA in UCI mode"""
    try:
        engine = UCIEngine()
        engine.run()
    except Exception as e:
