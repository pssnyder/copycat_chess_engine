...
class CopycatChessEngine:
    ...
    def __init__(self, results_dir="results"):
        ...
        print("Copycat Chess Engine initialized with data analytics")
    ...
    def select_move(self, board: chess.Board, remaining_time: Optional[float] = None) -> chess.Move:
        ...
        # No legal moves
        if not legal_moves:
            return chess.Move.null()  # Return a null move instead of None
        ...
        # Score each move
        scored_moves = [(move, self.score_move(move, board)) for move in legal_moves]
        ...
        # Select from top moves with probability proportional to score
        total_score = sum(score for _, score in top_moves)
        if total_score > 0:
            probabilities = [score / total_score for _, score in top_moves]
            selected_index = np.random.choice(len(top_moves), p=probabilities)
            selected_move = top_moves[selected_index][0]
        else:
